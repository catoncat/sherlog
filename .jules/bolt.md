...
## 2025-05-18 - Avoid node:path relative() overhead in tight loops\n**Learning:** Found that using `relative(root, file.filePath)` inside a loop iterating over hundreds of thousands of files involves heavy path parsing and normalization which dominates the CPU profile. This is unnecessary when all files are already collected under the resolved root.\n**Action:** Use a fast string slice `file.filePath.slice(prefixLength)` instead of `relative()` when paths are already absolute and guaranteed to be under the root. This drops generation time significantly (e.g., ~65% faster).
