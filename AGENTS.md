...
- Skill 发布层：`skill-packages/sherlog` 只是发布源码；全局安装版 skill 必须用 `npx skills add catoncat/sherlog --full-depth --skill sherlog -g -a codex -y` 从 GitHub 更新，不能 rsync dirty tree 或本地 symlink 冒充发布。
