...
CLI 改动需要对外生效时，走正式发布流程：bump `package.json` 版本 → `npm run check` → commit / Mainline seal → push `main` → push `v<version>` tag 或触发 release workflow → 回读 GitHub Actions 与 `npm view @act0r/cxs version`。发布成功后才更新本机 CLI，优先使用用户既有包管理器安装 registry 版；先查 shim 和包管理器来源，不要猜。

如果只完成源码提交但尚未发布，必须明确说“全局 `cxs` CLI 仍是旧发布版，不包含本次改动”。不要声称本机 CLI 已更新。
...
- 改 CLI 行为时，同步更新 `skill-packages/cxs`，并判断是否需要正式 CLI release；未 release 前只能用 `npm run cxs -- <args>` 验证 checkout，不要更新或覆盖全局 CLI
- 验证当前 checkout 的未发布代码时，用 `npm run cxs -- <args>`
- 验证已安装 / 已发布行为时，用 `cxs <args>`，并先核对 `command -v cxs` 与 `cxs --version`
- 全局 `cxs` skill 通过 `npx skills add` 更新；不要再创建 `cxsd` skill 或 symlink
...
- dev-only skill 源码在仓库内：`dev/skills/cxs-dogfood/source.md`；本机安装路径 `~/.agents/skills/cxs-dogfood/SKILL.md` 应通过 `scripts/install-dev-skills.sh` 指向该源码文件
...
2. 对失败 case 用 `npm run cxs -- find ... --json`、`npm run cxs -- read-range ... --json` 或 `npm run cxs -- read-page ... --json` 复现
3. 先判断问题层级：
...
- `npm run check`
- 必要时补一条 CLI 烟测，例如 `npm run cxs -- status --json` 或 `npm run cxs -- find "<query>" --json`
- 涉及 skill 通道时，验证 `npx skills ls -g --json` 和 `cxs --help`
- 涉及发布 / 安装态时，回读 `npm view @act0r/cxs version`、`which -a cxs`、`cxs --version`，并说明本机 CLI 是否已经来自 registry 发布版
