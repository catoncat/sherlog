...
| mutation: 建索引或更新 coverage | `shlog sync --cwd/--root/--selector` | 普通检索不要 `--prune`;只有用户明确要求清理已消失 source 的旧索引记录才用 |
...
# skill-sync: distributable sherlog skill package, zero-result coverage retry workflow, 2026-06-02
