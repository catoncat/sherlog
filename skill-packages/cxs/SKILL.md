...
"${CXS_BIN:-cxs}" status --help   # 应显示 --source <id> 且 public: codex
```
...
所有固定命令都可带 `--source <id>`；当前只用 `codex`，省略等价于 `--source codex`。遇到 `unsupported_source`，不要改用 raw source root，也不要声称 Claude Code 支持已发布。
