...
| Public CLI rejection preserved | CLI smoke and tests show `--source claude-code` rejected | pass: `npm run shlog -- status --source claude-code --json` exited 1 with `unsupported_source`. |
| Public selector rejection preserved | CLI smoke and tests show selector JSON requesting `claude-code` rejected | pass: `npm run shlog -- sync --selector '{"source":"claude-code",...}' --json` exited 1 with `unsupported_source`. |
