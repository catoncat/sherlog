...
Aligned current-state docs and release `Sherlog` skill source with the implemented source foundation. Public wording now says `codex` is the only public source, omitted `--source` means Codex, and `claude-code` is reserved/non-public. No Claude Code adapter, global skill update, npm release, installed CLI update, push, or PR was performed.
...
- `skill-packages/sherlog/references/advanced-queries.md`
- `src/types.ts`
...
- Skill source layer: `skill-packages/sherlog` source is updated in this checkout only.
- CLI release layer: no npm publish, tag, release workflow, or registry verification was performed.
- Installed local layer: no global `Sherlog`, PATH shim, or installed global skill was modified. Installed release behavior may lag this checkout until a future release/install workflow runs.

## Commit And Mainline Seal Status

- Commit status at handoff write time: pending; intended commit message `docs(Sherlog): 对齐 source 文档和 skill`.
...
- E1 should not infer global `Sherlog` or installed skill behavior from this checkout.
- E1 should keep `claude-code` reserved/non-public unless a later workflow explicitly implements and publishes it.

## Noise Events

- First `npm run shlog -- --help` failed because `node_modules` was absent (`tsx: command not found`); fixed with local `npm ci`.
